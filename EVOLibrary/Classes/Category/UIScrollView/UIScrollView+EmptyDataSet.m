...
- (UIButton *)button
{
    if (!_button)
    {
        ...
        _button.layer.borderColor = EVOHexColor(0xF2223B).CGColor;
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        CGFloat buttonPadding = (width-buttonWidth)/2.0;
        ...
        if ([self canShowButton]) {
            ...
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(buttonPadding@750)-[button(buttonWidth)]-(buttonPadding@750)-|"
                                                                                     options:0 metrics:metrics views:views]];
        }
        // or removes from its superview
        else {
